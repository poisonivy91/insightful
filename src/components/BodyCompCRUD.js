...
function BodyCompCRUD(){
    ...
    return <div className = "BodyCompCRUD">
        ...
        {users.map((user)=> {
            return <div><h3>Name: {user.name}</h3><h3>Age: {user.age}</h3> <button onClick={()=> {updateUser(user.id, user.age)}}> Increase Age</button><button onClick={() => {deleteUser(user.id)}}></button></div>})}</div>;
}
